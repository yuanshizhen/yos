...
export function LoginModal({ isOpen, onSuccess, onError }: Props) {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const user = loginRes.data.user as {
        id: string;
        username: string;
        avatarUrl?: string;
      };
    ...
    >
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-md rounded-[26px] border border-white/75 bg-white/78 p-6 shadow-[0_20px_70px_rgba(15,23,42,0.25)] backdrop-blur-2xl animate-[floatIn_.4s_ease-out]">
          <DialogTitle className="text-xl font-semibold tracking-[0.2px] text-slate-900">
            登录 YOS
          </DialogTitle>
          <p className="mt-1 text-xs text-slate-500">
            已填入超级管理员账号，请先登录后使用系统。
          </p>
