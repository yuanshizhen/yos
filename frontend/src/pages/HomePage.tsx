...
export function HomePage({ user, onAvatarUploaded }: { user: AuthUser; onAvatarUploaded: (url: string) => void }) {
    ...
    <div className="rounded-[24px] border border-white/80 bg-white/60 p-5 shadow-[0_20px_70px_rgba(148,163,184,0.2)] backdrop-blur-2xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-xs text-slate-600">亮色 Apple Glass 模式已启用。二级菜单打开时内容区会模糊化，三级菜单跟随二级 hover 动态显示。</p>
        </div>
        <label className="inline-block">
          <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={onAvatarChange} disabled={avatarUploading} />
          <span><GlassButton>{avatarUploading ? '上传中...' : '上传头像'}</GlassButton></span>
        </label>
      </div>
      ...
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-white/80 bg-white/70 p-3">
        {user.avatarUrl ? <img src={user.avatarUrl} className="h-12 w-12 rounded-full object-cover" /> : <div className="h-12 w-12 rounded-full bg-gradient-to-br from-violet-200 to-cyan-100" />}
        <div>
          <p className="text-xs text-slate-500">当前用户</p>
          <p className="text-sm font-semibold text-slate-900">{user.username}</p>
          {avatarMessage && <p className="text-xs text-cyan-700">{avatarMessage}</p>}
        </div>
      </div>
    </div>
  )
